...
export default function DashboardPage() {
  ...
  const fetchStats = async () => {
    try {
      ...
      const itemsInStock =
        barangStokRes.data.data?.filter((item: any) => item.qty_akhir > 0)
          .length || 0;
      ...
      <div className="bg-white shadow rounded-lg p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <Link
            ...
            className="text-center p-4 bg-blue-50 hover:bg-blue-100 rounded-lg transition"
          >
            <div className="text-blue-600 font-semibold">Master Barang</div>
          </Link>
          <Link
            href="/dashboard/stok"
            className="text-center p-4 bg-green-50 hover:bg-green-100 rounded-lg transition"
          >
            <div className="text-green-600 font-semibold">Stock</div>
          </Link>
          <Link
            href="/dashboard/pembelian"
            className="text-center p-4 bg-yellow-50 hover:bg-yellow-100 rounded-lg transition"
          >
            <div className="text-yellow-600 font-semibold">Pembelian</div>
          </Link>
          <Link
            href="/dashboard/penjualan"
            className="text-center p-4 bg-purple-50 hover:bg-purple-100 rounded-lg transition"
          >
            <div className="text-purple-600 font-semibold">Penjualan</div>
          </Link>
          <Link
            href="/dashboard/history"
            className="text-center p-4 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition"
          >
            <div className="text-indigo-600 font-semibold">History</div>
          </Link>
        </div>
      </div>
    </div>
